...
create table GUCStudentPhoneNumber (
    id int  FOREIGN Key REFERENCES GucianStudent
...
create table NonGUCStudentPhoneNumber (
    id int  FOREIGN Key REFERENCES NonGucianStudent
     ON delete cascade on update cascade,
    phone int,
    PRIMARY KEY(id,phone)
    )
...
begin
set @success =1
-- check user type 0-->Student,1-->Admin,2-->Supervisor ,3-->Examiner,4-->nonGucian
...
go
declare @suc bit , @typ int
exec userlogin 19,'000',@suc output,@typ OUTPUT
print @suc
 print @typ
go
...
RAISERROR('Thesis does not exist',16,1);
...
begin
select DISTINCT s.firstName,s.lastName,t.years
...
GO
-- hazem 
create proc listStudentThesis
@studentID int 
as

if exists(select g.serial_no from GUCianStudentRegisterThesis g where g.sid = @studentID )
and exists (select t.serialNumber from thesis t inner join GUCianStudentRegisterThesis g on t.serialNumber = g.serial_no 
where g.sid = @studentID )
begin 

select t.* from thesis t inner join
 GUCianStudentRegisterThesis g on t.serialNumber = g.serial_no 
where g.sid = @studentID 


end

else if exists(select g.serial_no from nonGUCianStudentRegisterThesis g where g.sid = @studentID)
and exists (select t.serialNumber from thesis t inner join nonGUCianStudentRegisterThesis g on t.serialNumber = g.serial_no 
where g.sid = @studentID )
begin 

select t.* from thesis t inner join
 nonGUCianStudentRegisterThesis g on t.serialNumber = g.serial_no 
where g.sid = @studentID 
...
go
create proc liststudentongoing
@studentID int 
AS
declare @gucian int
if(exists(
...
if(@gucian='1')
begin
SELECT Concat('serial number= ',t.serialNumber,', title: ',t.title ) as title2 
from 
  GUCianStudentRegisterThesis g 
inner join Thesis t on t.serialNumber = g.serial_no
where g.sid = @studentID and t.startDate <CURRENT_TIMESTAMP and t.endDate>CURRENT_TIMESTAMP
END
else if(@gucian='0')
 BEGIN
 SELECT Concat('serial number= ',t.serialNumber,', title: ',t.title ) as title2 
from 
  nonGUCianStudentRegisterThesis g 
inner join Thesis t on t.serialNumber = g.serial_no
where g.sid = @studentID and t.startDate <CURRENT_TIMESTAMP and t.endDate>CURRENT_TIMESTAMP
END
...
create proc liststudentongoinghelper
@studentID int
as
declare @gucian int
if(exists(
...
if(@gucian='1')
begin
select t.serialnumber from GUCianStudentRegisterThesis g inner join
 Thesis t on t.serialNumber = g.serial_no
 where g.sid = @studentID and t.startDate <CURRENT_TIMESTAMP and t.endDate>CURRENT_TIMESTAMP
 end
 else if(@gucian='0')
 BEGIN
 select t.serialnumber from nonGUCianStudentRegisterThesis g inner join
 Thesis t on t.serialNumber = g.serial_no
 where g.sid = @studentID and t.startDate <CURRENT_TIMESTAMP and t.endDate>CURRENT_TIMESTAMP
 end
GO

exec liststudentongoinghelper 4

go
create proc studentlistcourses 
@studentID int 
as 
select c.id,c.code ,ng.grade from Course c inner join NonGucianStudentTakeCourse ng on c.id = ng.cid
...
go
 ...
 @id int
 as
 select * from examiner e where e.id=@id 
 go 

 
 create proc viewEverythingExaminer
 @id int

 as

select th.title as thesisTitle , sup.name as SupervisorName, gucs.firstname + ' ' + gucs.lastname as studentname
from ExaminerEvaluateDefense e 
inner join Thesis th on e.serialNo=th.serialNumber 
inner join GUCianStudentRegisterThesis gucth on th.serialNumber=gucth.serial_no
inner join GucianStudent gucs on gucth.sid= gucs.id
inner join Supervisor sup on sup.id = gucth.supid
where e.examinerId =@id and th.defenseDate < GETDATE()
union
select th.title as thesisTitle , sup.name as SupervisorName , nongucs.firstname + ' ' + nongucs.lastname as studentname
from ExaminerEvaluateDefense e 
inner join Thesis th on e.serialNo=th.serialNumber 
inner join NonGUCianStudentRegisterThesis nongucth on th.serialNumber=nongucth.serial_no
inner join NonGucianStudent nongucs on nongucth.sid= nongucs.id
inner join Supervisor sup on sup.id = nongucth.supid
where e.examinerId =@id and th.defenseDate < GETDATE()
...
update ExaminerEvaluateDefense 
set comment = @comments
where serialNo = @ThesisSerialNo and  ExaminerEvaluateDefense.[date]= @DefenseDate and examinerId = @id 

go

create proc AddDefenseGrade
@id int , @ThesisSerialNo int , @DefenseDate Datetime , @grade decimal(4,2)
as
if exists(select * from ExaminerEvaluateDefense ed where ed.[date] = @DefenseDate and ed.serialNo = @ThesisSerialNo and ed.examinerId = @id)
begin
update Defense
set grade = @grade
where serialNumber = @ThesisSerialNo and date = @DefenseDate 
...
inner join ExaminerEvaluateDefense exd on th.serialNumber = exd.serialNo 
where exd.examinerId = @id and th.title like '%'+@word+'%' 
...
-- Adhan -----------------------
CREATE PROCEDURE checkid4
@studentID int ,
@valid BIT OUTPUT
AS
IF exists( (select * from GUCianStudentRegisterThesis g where g.sid=@studentID)
UNION
(SELECT * FROM NonGUCianStudentRegisterThesis ng  where ng.sid=@studentID))
BEGIN
SET @valid=1
END
ELSE 
...
from GUCianProgressReport
where thesisSerialNumber = @ThesisSerialNo
order by GUCianProgressReport.[no] desc
)
if(@gucianEval = 0)
begin
SET @ok =0
...
from NonGUCianProgressReport
where thesisSerialNumber = @ThesisSerialNo
order by  NonGUCianProgressReport.[no] desc
)
if(@nonGucianEval = 0)
begin
SET @ok=0
...
ELSE 
RAISERROR('grade is less than 50',16,1)
go

CREATE PROC insintdef
@date DATETIME,
@serialNo int,
@examinerId INT
as
INSERT into ExaminerEvaluateDefense VALUES (@date,@serialNo,@examinerId,'');

GO
CREATE PROC chechser
@ThesisSerialNo INT,
@bit BIT OUTPUT
AS
